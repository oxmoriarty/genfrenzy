...
interface PlayerRow {
  ...
  correctAnswers: number; partialAnswers: number;
  incorrectAnswers: number; questionResults: any[];
}
interface LBEntry {
  playerId: string; username: string; score: number; rank: number;
  correctAnswers: number; partialAnswers: number;
}
...
// ─── CSV export ───────────────────────────────────────────────────────────────
function exportCSV(players: PlayerRow[], questions: any[], theme: string, quizCode: string) {
  const sorted = [...players].sort((a,b) => b.score - a.score).map((p,i) => ({...p, rank:i+1}));
  const CRLF = '\r\n';
  ...
  const qRH = questions.map((_:any,i:number) => 'Q'+(i+1)+' Result');
  const qPH = questions.map((_:any,i:number) => 'Q'+(i+1)+' Points');
  csv += row('Rank','Username','Total Score',...qRH,...qPH);
  for (const p of sorted) {
    const qr = p.questionResults || [];
    const res = questions.map((_:any,qi:number) => {
    ...
    csv += row(p.rank, p.username, p.score, ...res, ...pts);
  }
  csv += CRLF;
  csv += row('QUESTIONS');
  ...
  for (let qi=0; qi<questions.length; qi++) {
    ...
    const co = (q.correctIndices as number[]).map((ci:number) => q.options[ci]||('Option '+(ci+1))).join(' | ');
    csv += row('Q'+(qi+1), q.text||('Question '+(qi+1)), q.isMultipleChoice?'Multiple Choice':'Single Choice', co);
  }
  const bom = '\uFEFF';
  const blob = new Blob([bom+csv], { type:'text/csv;charset=utf-8;' });
  const url  = URL.createObjectURL(blob);
  const a    = document.createElement('a');
  a.href=url; a.download='GenFrenzy_'+quizCode+'_Results.csv';
...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    const stored = localStorage.getItem('gf_admin_session');
    if (!stored) return;
    try {
      const sess = JSON.parse(stored);
      if (!sess.code || !sess.password) return;
      // Try to rejoin dashboard
      sk.emit('admin_rejoin', { password: sess.password, code: sess.code }, (res: any) => {
        if (res.success) {
          setQuizCode(sess.code);
          setQuizPwd(sess.password);
  ...
  const refresh = useCallback(() => {
    ...
    sk.emit('admin_get_dashboard', { code: codeRef.current }, (res: any) => {
      ...
      setTotalQ(res.quiz.totalQuestions);
    });
  }, []);
  ...
  useEffect(() => {
    ...
    sk.on('connect', () => {
      if (codeRef.current && pwdRef.current) {
        sk.emit('admin_rejoin', { password: pwdRef.current, code: codeRef.current }, () => {});
        refresh();
      }
    });
  ...
  const login = () => {
    ...
    const doVerify = () => {
      const timer = setTimeout(() => {
        ...
        setLoginErr('Server not responding. Check your connection and try again.');
      }, 8000);
    ...
    if (sk.connected) {
      ...
      const connectTimer = setTimeout(() => {
        setLoginLoading(false);
        setLoginErr('Cannot connect to server. Make sure the backend is running.');
      }, 8000);
      sk.once('connect', () => {
  ...
  const create = () => {
    ...
    if (questions.some(q => q.options.some(o => !o.trim()))) {
      setCreateErr('Fill all option fields'); return;
    }
    if (questions.some(q => q.correctIndices.length === 0)) {
    ...
    sk.emit('admin_create_quiz', { password, theme, questions }, (res: any) => {
      ...
      if (res.success) {
        ...
        // Save admin session for reconnect
        localStorage.setItem('gf_admin_session', JSON.stringify({ code: res.code, password }));
        setView('dashboard');
      } else {
        setCreateErr(res.error || 'Failed to create quiz');
      }
    });
  };
  ...
  const startQuiz = () => {
    setStarting(true);
    sk.emit('admin_start_quiz', { password: quizPwd, code: quizCode }, (res: any) => {
      setStarting(false);
  ...
  const doExport = () => {
    setExporting(true);
    sk.emit('admin_export_data', { password: quizPwd, code: quizCode }, (res: any) => {
      setExporting(false);
  ...
  // ── Question helpers ───────────────────────────────────────────────────────
  const updateQ   = (i:number, f:keyof QInput, v:any) =>
    setQuestions(qs => { const n=[...qs]; (n[i] as any)[f]=v; return n; });
  const updateOpt = (qi:number, oi:number, v:string) =>
    setQuestions(qs => { const n=[...qs]; n[qi].options[oi]=v; return n; });
  ...
  const rowSt  = { display:'flex' as const, alignItems:'center' as const, gap:10, padding:'10px 18px', borderBottom:'1px solid #111118' as const };
    ...
    <div style={{ minHeight:'100vh', background:'#0A0A0F', display:'flex', alignItems:'center', justifyContent:'center', padding:24, position:'relative' }}>
      ...
      <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }}
        ...
        <div style={{ ...card, padding:28 }}>
          ...
          {loginErr && (
            <div style={{ display:'flex', alignItems:'center', gap:6, fontSize:12, color:'#FF6B82', marginBottom:12 }}>
              <AlertCircle size={12}/> {loginErr}
            </div>
          )}
          <motion.button onClick={login} disabled={loginLoading}
            whileHover={!loginLoading?{scale:1.015}:{}} whileTap={!loginLoading?{scale:.985}:{}}
            style={{ width:'100%', padding:'13px', borderRadius:12, border:'none',
              cursor:loginLoading?'default':'pointer', fontWeight:600, fontSize:14, color:'#fff',
              background:loginLoading?'rgba(59,110,255,.4)':'linear-gradient(135deg,#3B6EFF,#6C47FF)',
              boxShadow:loginLoading?'none':'0 8px 28px rgba(108,71,255,.4)',
              display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
            {loginLoading ? <><Loader2 size={15} className="anim-spin"/> Verifying…</> : 'Enter Dashboard'}
          </motion.button>
        </div>
      </motion.div>
    </div>
      ...
      <div style={{ position:'sticky', top:0, zIndex:10, background:'rgba(10,10,15,.97)', backdropFilter:'blur(16px)', borderBottom:'1px solid #25252E', padding:'14px 24px' }}>
        <div style={{ maxWidth:720, margin:'0 auto', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          ...
          <motion.button onClick={create} disabled={creating}
            whileHover={!creating?{scale:1.016}:{}} whileTap={!creating?{scale:.984}:{}}
      ...
      <div style={{ maxWidth:720, margin:'0 auto', padding:'24px 24px 80px', display:'flex', flexDirection:'column', gap:16 }}>
        {createErr && (
          <div style={{ display:'flex', alignItems:'center', gap:8, padding:'12px 16px', borderRadius:12,
            background:'rgba(255,77,106,.1)', border:'1px solid rgba(255,77,106,.25)', color:'#FF6B82', fontSize:13 }}>
            ...
            <motion.div key={qi} initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }}
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16 }}>
                <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                  <div style={{ width:26, height:26, borderRadius:8, background:'rgba(108,71,255,.2)',
                    display:'flex', alignItems:'center', justifyContent:'center',
                    fontSize:12, fontWeight:700, color:'#6C47FF', fontFamily:'var(--font-jb)' }}>{qi+1}</div>
                  <span style={{ fontSize:12, color:'#9898AA', fontFamily:'var(--font-jb)', textTransform:'uppercase', letterSpacing:'.1em' }}>Question {qi+1}</span>
                  {isMulti && (
                    <span style={{ padding:'2px 8px', borderRadius:100, fontSize:9, fontWeight:600,
                      background:'rgba(155,89,255,.18)', color:'#9B59FF',
                      border:'1px solid rgba(155,89,255,.3)', fontFamily:'var(--font-jb)' }}>MULTI-CORRECT</span>
                  )}
                </div>
                {questions.length > 1 && (
                  <button onClick={()=>removeQ(qi)}
                    style={{ display:'flex', alignItems:'center', gap:4, background:'none', border:'none',
                      cursor:'pointer', color:'#6B6B80', fontSize:12, padding:'4px 8px', borderRadius:8, transition:'color .15s' }}
                    onMouseOver={e=>(e.currentTarget.style.color='#FF4D6A')}
                  ...
                  <div style={{ position:'relative', borderRadius:12, overflow:'hidden', border:'1px solid #25252E', background:'#111118' }}>
                    ...
                    <button onClick={()=>updateQ(qi,'imageBase64',null)}
                      style={{ position:'absolute', top:8, right:8, width:28, height:28, borderRadius:'50%',
                        background:'rgba(0,0,0,.7)', border:'1px solid #3A3A48', cursor:'pointer',
                        display:'flex', alignItems:'center', justifyContent:'center', color:'#F0F0F8' }}>
                      <X size={13}/>
                    </button>
                  </div>
                ) : (
                  <label style={{ display:'flex', alignItems:'center', gap:8, padding:'11px 14px',
                    borderRadius:12, border:'1.5px dashed #3A3A48', cursor:'pointer',
                    color:'#6B6B80', fontSize:13, transition:'border-color .2s, color .2s' }}
                      ...
                      <div key={oi} style={{ display:'flex', alignItems:'center', gap:8 }}>
                        <button onClick={()=>toggleCorrect(qi,oi)}
                          style={{ width:24, height:24, borderRadius:7, flexShrink:0, cursor:'pointer',
                            border:`2px solid ${isCorrect?'#00C98D':'#3A3A48'}`,
                            background:isCorrect?'#00C98D':'transparent',
                            display:'flex', alignItems:'center', justifyContent:'center', transition:'all .15s' }}>
                          {isCorrect && <Check size={12} color="#fff" strokeWidth={2.5}/>}
                        </button>
                        <div style={{ flex:1, display:'flex', alignItems:'center', gap:6,
                          background:'#111118', border:'1px solid #25252E', borderRadius:10, padding:'0 10px', height:40 }}>
                          ...
                          <input value={opt} onChange={e=>updateOpt(qi,oi,e.target.value)}
                            ...
                            style={{ flex:1, background:'none', border:'none', outline:'none',
                              fontSize:13, color:'#F0F0F8', fontFamily:'var(--font-plus)' }}/>
                        </div>
                      </div>
                    );
                  })}
              ...
              {q.correctIndices.length > 0 && (
                <p style={{ fontSize:11, color:'#00C98D', fontFamily:'var(--font-jb)' }}>
        ...
        <motion.button onClick={addQ}
          whileHover={{ borderColor:'#6C47FF', color:'#9B59FF' }}
          style={{ padding:'14px', borderRadius:18, border:'1.5px dashed #25252E', background:'none',
            cursor:'pointer', fontSize:13, color:'#6B6B80', display:'flex',
            alignItems:'center', justifyContent:'center', gap:8, transition:'all .2s' }}>
        ...
        <div style={{ maxWidth:960, margin:'0 auto', display:'flex', flexWrap:'wrap', alignItems:'center', justifyContent:'space-between', gap:16 }}>
          <div>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10, flexWrap:'wrap' }}>
              ...
              <code style={{ fontSize:18, fontWeight:700, color:'#F0F0F8', fontFamily:'var(--font-jb)',
                letterSpacing:'.2em', background:'rgba(255,255,255,.06)', padding:'3px 12px', borderRadius:8 }}>
                {quizCode}
              </code>
              <button onClick={copyCode}
                style={{ display:'flex', alignItems:'center', gap:5, padding:'5px 10px', borderRadius:8,
                  border:'1px solid #25252E', background:'none', cursor:'pointer',
                  fontSize:11, color:'#9898AA', transition:'all .2s' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            ...
            {status==='ended' && (
              <motion.button onClick={doExport} disabled={exporting}
                ...
                style={{ display:'flex', alignItems:'center', gap:8, padding:'11px 22px', borderRadius:14, border:'none',
                  cursor:exporting?'default':'pointer', fontWeight:700, fontSize:14, color:'#fff',
            ...
            {status==='waiting' && (
              <motion.button onClick={startQuiz} disabled={starting}
                ...
                style={{ display:'flex', alignItems:'center', gap:8, padding:'12px 28px', borderRadius:14, border:'none',
                  cursor:starting?'default':'pointer', fontWeight:700, fontSize:15, color:'#fff',
            ...
            <div style={{ maxHeight:340, overflowY:'auto' }}>
              {players.length===0 ? (
                ...
                <motion.div key={p.id} initial={{ opacity:0, x:-8 }} animate={{ opacity:1, x:0 }}
                  transition={{ delay:i*.04 }} style={{ ...rowSt }}>
                  <div style={{ width:28, height:28, borderRadius:9, background:'rgba(108,71,255,.18)',
                    display:'flex', alignItems:'center', justifyContent:'center',
                    fontSize:11, fontWeight:700, color:'#6C47FF', flexShrink:0 }}>
          ...
          <div style={{ ...card, padding:0, overflow:'hidden' }}>
              ...
              {status==='ended' && (
                <button onClick={doExport} disabled={exporting}
                  style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:5,
                    padding:'5px 10px', borderRadius:8, border:'1px solid rgba(0,212,180,.3)',
                    background:'rgba(0,212,180,.1)', cursor:'pointer', fontSize:11,
                    color:'#00D4B4', fontWeight:600 }}>
            ...
            <div style={{ maxHeight:340, overflowY:'auto' }}>
              {lb.length===0 ? (
                ...
                <div key={e.playerId} style={{ ...rowSt, background:e.rank<=3?'rgba(108,71,255,.06)':'transparent' }}>
                  <span style={{ width:24, textAlign:'center', fontSize:11, fontWeight:700,
                    fontFamily:'var(--font-jb)', flexShrink:0,
