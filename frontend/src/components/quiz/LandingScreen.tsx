    ...
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden"
      ...
      {[
        ...
        <motion.div key={i}
          ...
          style={{ width:o.w, height:o.h, background:o.c, ...Object.fromEntries(
            ['top','right','bottom','left'].filter(k => k in o).map(k => [k, (o as any)[k]])
          )}} />
      ))}
      ...
      <div className="relative z-10 w-full max-w-[420px] mx-auto px-6 py-16">
        ...
        <motion.div
          ...
          <p style={{ fontSize:16, color:'#9898AA', fontWeight:500, letterSpacing:'0.01em' }}>
        ...
        {/* Form card */}
        <motion.div
          initial={{ opacity:0, y:22, scale:.97 }} animate={{ opacity:1, y:0, scale:1 }}
          transition={{ delay:.25, duration:.4, ease:'easeOut' }}>
          <div className="rounded-2xl p-6"
            style={{ background:'#18181F', border:'1px solid #25252E', boxShadow:'0 2px 0 rgba(255,255,255,.04), 0 24px 64px rgba(0,0,0,.65)' }}>
            <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
              ...
              <div>
                ...
                <div style={{ position:'relative' }}>
                  <Hash size={14} style={{ position:'absolute', left:14, top:'50%', transform:'translateY(-50%)', color:'#3A3A48', pointerEvents:'none' }} />
              ...
              <div>
                ...
                <div style={{ position:'relative' }}>
                  <User size={14} style={{ position:'absolute', left:14, top:'50%', transform:'translateY(-50%)', color:'#3A3A48', pointerEvents:'none' }} />
              ...
              <AnimatePresence>
                {error && (
                  <motion.div
                    ...
                    style={{ display:'flex', alignItems:'center', gap:8, padding:'10px 14px', borderRadius:10, background:'rgba(255,77,106,.1)', border:'1px solid rgba(255,77,106,.25)', color:'#FF6B82', fontSize:13 }}>
              ...
              <motion.button onClick={join} disabled={loading}
                style={{
                  ...
                  background: loading ? 'rgba(108,71,255,.4)' : 'linear-gradient(135deg,#3B6EFF 0%,#6C47FF 55%,#9B59FF 100%)',
                  boxShadow: loading ? 'none' : '0 8px 32px rgba(108,71,255,.45), inset 0 1px 0 rgba(255,255,255,.12)',
                ...
                {!loading && (
                  <div style={{ position:'absolute', inset:0, background:'linear-gradient(105deg,transparent 30%,rgba(255,255,255,.1) 50%,transparent 70%)', backgroundSize:'200% 100%', animation:'shimmer 2.5s linear infinite', pointerEvents:'none' }} />
                )}
                {loading
