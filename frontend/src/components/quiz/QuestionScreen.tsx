...
export default function QuestionScreen() {
  const {
    ...
    hasAnswered, selectedIndices, setHasAnswered, toggleSelected, setSelectedIndices,
  } = useGameStore();

  const tlRef = useRef(timeLeft);
  tlRef.current = timeLeft;

  // Submit final answer(s) to server
  ...
  // Single-choice: tap once → immediate submit
  ...
  const warning = phase === 'question_options' && timeLeft <= 10 && timeLeft > 5;
  ...
  return (
    <div style={{ position:'relative', minHeight:'100vh', display:'flex', flexDirection:'column', background:'#0A0A0F' }}>
      ...
      <div style={{ height:3, background:'#111118', flexShrink:0 }}>
        <motion.div animate={{ scaleX: phase === 'question_options' ? pct : 1 }}
          transition={{ duration:.4 }}
          style={{ height:'100%', transformOrigin:'left', background:`linear-gradient(90deg,${tc},${tc}88)` }} />
      </div>

      {/* Header */}
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 20px', maxWidth:700, margin:'0 auto', width:'100%' }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <div style={{ width:32, height:32, borderRadius:10, background:'linear-gradient(135deg,#3B6EFF,#6C47FF)', display:'flex', alignItems:'center', justifyContent:'center' }}>
            <span style={{ color:'#fff', fontWeight:800, fontSize:13 }}>G</span>
          </div>
          ...
          {isMultipleChoice && phase === 'question_options' && (
            <span style={{ padding:'3px 10px', borderRadius:100, fontSize:10, fontWeight:600,
              background:'rgba(155,89,255,.18)', color:'#9B59FF', fontFamily:'var(--font-jb)',
        ...
        <AnimatePresence mode="wait">
          {phase === 'question_options' && (
            <motion.div key="timer" initial={{ scale:.6, opacity:0 }} animate={{ scale:1, opacity:1 }}
              ...
              <motion.div animate={urgent ? { scale:[1,1.1,1] } : {}}
      ...
      {/* Body */}
      <div style={{ flex:1, display:'flex', flexDirection:'column', padding:'0 16px 32px', maxWidth:700, margin:'0 auto', width:'100%' }}>

        {/* Question card */}
        <AnimatePresence mode="wait">
          <motion.div key={`q${questionIndex}-${phase}`}
            initial={{ opacity:0, y:14 }} animate={{ opacity:1, y:0 }}
            exit={{ opacity:0, y:-14 }} transition={{ duration:.28 }}
            style={{ borderRadius:22, padding:'22px 22px 18px', marginBottom:16, position:'relative',
              overflow:'hidden', background:'#18181F', border:'1px solid #25252E',
              boxShadow:'0 4px 32px rgba(0,0,0,.55)', minHeight:100 }}>
            ...
            <div style={{ position:'relative', zIndex:1 }}>
              <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:14 }}>
                <div style={{ width:3, height:18, borderRadius:2, background:'linear-gradient(180deg,#3B6EFF,#6C47FF)' }} />
              ...
              {currentQuestion?.imageBase64 && (
                <div style={{ marginTop: currentQuestion.text ? 14 : 0, borderRadius:14, overflow:'hidden',
                  border:'1px solid #25252E', background:'#111118',
                  ...
                  return (
                    <motion.div key={i}
                      ...
                      <div style={{ display:'flex', alignItems:'flex-start', gap:12, padding:'14px' }}>
                        {/* Checkbox (multi) or letter badge (single) */}
                        {isMultipleChoice ? (
                          ...
                          <div style={{ flexShrink:0, width:28, height:28, borderRadius:8,
                            ...
                            background: isSel ? o.c : `${o.c}28`, color: isSel ? '#fff' : o.c }}>
                            {o.L}
                          </div>
                        )}
                  ...
                  <motion.button
                    ...
                    style={{
                      ...
                      background: selectedIndices.length === 0
                        ? 'rgba(108,71,255,.2)'
                        : 'linear-gradient(135deg,#3B6EFF,#6C47FF)',
